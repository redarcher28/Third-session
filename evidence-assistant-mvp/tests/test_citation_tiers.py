...
class CitationTierTests(unittest.TestCase):
    ...
    def test_trial_registry_completed_vs_recruiting(self) -> None:
        ...
        level_unk, eligible_unk = classify_trial_registry("UNKNOWN")
        level_empty, eligible_empty = classify_trial_registry("")
        self.assertEqual(level_done, "other")
        self.assertTrue(eligible_done)
        self.assertFalse(eligible_rec)
        self.assertFalse(eligible_unk)
        self.assertFalse(eligible_empty)
        # 已完成 ≠ 可作疗效结论：等级仍为 other，不得升格为 rct
        self.assertNotEqual(level_done, "rct")
