...
对 data/eval/clinical_questions.json 中的临床题逐题运行 ask()，
检查：结构化大纲、引用存在性、引用校验、要点覆盖，并输出汇总。

用法:
    python scripts/check_clinical.py [--questions data/eval/clinical_questions.json]
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="赛道一临床自检")
    parser.add_argument(
        "--questions",
        type=Path,
        default=ROOT / "data" / "eval" / "clinical_questions.json",
    )
    ...
    violations = 0
    for it in items:
        resp = ask(it["question"], track="clinical")
        check = resp.citation_check or {}
        outline = check.get("clinical_outline") or {}
