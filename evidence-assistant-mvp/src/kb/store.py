...
def rebuild_collection_from_processed(reset: bool = True) -> int:
    ...
    """
    settings = get_settings()
    source_path = settings.processed_path / "documents_with_wiki.json"
    if not source_path.exists():
        source_path = settings.processed_path / "documents.json"
...
def export_store_stats(out_path=None) -> dict:
    ...
    """
    store = EvidenceStore()
    chunks = store.all_chunks_for_bm25(limit=20000)
    by_source = Counter(str(c.get("source") or "unknown") for c in chunks)
    by_level = Counter(str(c.get("evidence_level") or "other") for c in chunks)
    by_tag = Counter()
    by_year = Counter()
    for chunk in chunks:
        ...
        for tag in [t.strip() for t in tags.split(",") if t.strip()]:
            by_tag[tag] += 1
        year = chunk.get("year")
        by_year[str(year) if year not in (None, -1, "-1") else "unknown"] += 1
    ...
    stats = {
        ...
        "by_tag": dict(by_tag.most_common(50)),
    ...
    lines = [
        "# 知识库统计",
        "",
        f"- Chroma 向量条数: {stats['count']}",
        f"- 导出 chunk 条数: {stats['exported_chunks']}",
        "",
        "## 来源分布",
        "",
        "| source | count |",
    ...
    lines.extend(["", "## 证据等级分布", "", "| level | count |", "|---|---:|"])
    lines.extend(f"| {k} | {v} |" for k, v in by_level.most_common())
    lines.extend(["", "## 标签分布", "", "| tag | count |", "|---|---:|"])
    lines.extend(f"| {k} | {v} |" for k, v in by_tag.most_common(50))
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return stats
