...
- 新增本机 `/settings` 模型连接设置页：可配置 API 格式、Base URL、API Key、模型名和 reasoning effort；配置保存到用户目录私有运行时文件，热加载到 `LLMClient`，不要求手动编辑 `.env`。
...
结合本项目要求后的改造放在后端协议和系统 Prompt：Open WebUI 的模型选择器中提供“赛道一 · 临床证据”和“赛道二 · 健康营养”，分别映射到同一条证据流水线的不同预置 Prompt。回答正文保留 `[n]` 引用和伦理声明，适配层还会把本次实际采用的证据标题、证据等级、年份、摘要片段、原文链接、改写查询和引用校验状态追加为 Markdown“证据面板”，因此 Open WebUI 可以直接呈现原旧前端的可核对 RAG 结果；`/ask` 仍提供结构化 `citations/contexts/retrieval/citation_check`，不改变主聊天协议。
...
`LLM_MODEL=gpt-5.6-luna` 和 `LLM_REASONING_EFFORT=xhigh`。客户端会自动请求
...
`LLM_BASE_URL`、`LLM_MODEL` 和 `LLM_REASONING_EFFORT`；后者保存到用户目录的运行时
