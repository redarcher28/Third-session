...
- 启动脚本通过 `scripts/install_openwebui_bridge.py` 把模型连接卡片嵌入 Open WebUI 原生“设置 → 通用”页面；只配置 API 格式、Base URL、API Key 和模型名，配置保存到用户目录私有运行时文件并热加载到 `LLMClient`。桥接卡片沿用 Open WebUI 的灰黑表面、灰色边框和蓝色交互色，不改 A 组代码；`/settings` 仍保留为备用页。Prompt、检索数量和生成策略不在页面开放细调。
...
| `src/app/static/` | 无构建依赖的统一降级前端；主前端由独立 Open WebUI 提供，备用页通过 `/fallback` 访问；`openwebui_bridge.js/.css` 负责嵌入设置卡片。 |
| `scripts/configure_openwebui.py` | 以 SQLite 事务合并 Open WebUI Banner/Prompt Suggestions/水印，保留用户已有配置。 |
| `scripts/install_openwebui_bridge.py` | 将项目自己的模型连接卡片和原生风格样式注入 Open WebUI `loader.js`；启动时可重复执行，保留非项目 loader 备份。 |
...
预置了同样的无密钥模板。启动后打开 Open WebUI 的“设置 → 通用 → 证据模型连接”，
即可在主前端修改连接配置；`http://127.0.0.1:8000/settings` 仍是备用设置页。按
[ByeAPI 可用渠道页面](https://byeapi.top/available-channels)
...
# 也可以跳过编辑，启动后打开 Open WebUI → 设置 → 通用 → 证据模型连接
```

如果需要切换供应商，可以编辑 `.env`，也可以在 Open WebUI 的证据模型连接卡片（或备用设置页）修改 `LLM_API_FORMAT`、
...
- 流式传输修复：此前后端先等待完整 `ask()` 结果，再把全文切成假 SSE 分片；现改为 `LLMClient.stream_chat()` 消费 Responses/Anthropic/Chat Completions 的上游增量事件，由 RAG worker 回调实时转成标准 `chat.completion.chunk`，Sources 仍在回答完成后单独发送。
- 真实 ByeAPI 烟测：`POST /v1/chat/completions` 返回 `200 text/event-stream`；角色帧立即到达，约 2.9 秒后开始收到模型文本，随后多个不同时间戳的内容分片持续到达，最后发送 native Sources、结束帧和 `[DONE]`。
