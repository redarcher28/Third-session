...
OpenAI-compatible LLM 后，同一条链路会把检索上下文注入 synthesis Prompt，再由模型生成
回答；不需要改前端或另写一套 RAG。
...
- `conda run -p /Users/quentincrane/conda_envs/evidence_mvp python -m unittest -v tests.test_rag_contract`：4 项通过，覆盖 Prompt 注入、检索结果传递、引用校验和 Open WebUI 来源回显。
- `git diff --check`：通过。
